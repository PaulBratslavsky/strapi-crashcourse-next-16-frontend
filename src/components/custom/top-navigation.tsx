...
import { TestErrorButton } from "@/components/custom/test-error-button";
  ...
  return (
    <nav className="bg-[#F9F5F2] border-y-3">
      <div className="container h-14 max-w-6xl mx-auto flex items-stretch justify-between px-4 xl:px-0">
        {/* Logo */}
        <Link href={logoHref} className="text-2xl font-head border-x-3 px-4 border-black flex items-center">
          {logoImage ? (
            <Image
                ...
                >
                  <Avatar className="h-9 w-9">
                    {currentUser.image ? (
                      <Avatar.Image>
                        <Image
                          src={getStrapiMedia(currentUser.image.url)}
